...
We remember that Windows uses Unicode UTF-16 (see also ["Quals NDH 2018 - Where is my Purse ?"]({% post_url quals-NDH-18-whereismypurse %}), so we can try again with the `-el` (`-e` for "encoding" and `l` for "16-bit littleendian" which is UTF-16).
```console
