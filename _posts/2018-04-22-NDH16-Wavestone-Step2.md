...
## Solution 
The first step is to decode base64 encoded payload from [step 1](fixme_url). We obtain the following PowerShell 
```powershell
...
```
Oh ... more PowerShell, let's analyse it. `( $pSHome[21]+$PsHOme[30]+'x')` is just and obfuscated `IEX`, we can execute the third part of the payload to get the deobfucated one.
```powershell
...
```
Finaly ! The payload execute twe remote scripts. The first one is the **Invoke-AmsiBypass** from [Nishang](https://github.com/samratashok/nishang) and the second one is used to reflectively inject a PE in memory.
The second flag is **https://delivery.aperture-science.fr/dc1cca06661d3e14a343a2969b10a53a26bacb87.ps1**, it contains a RATS encoded in base64.