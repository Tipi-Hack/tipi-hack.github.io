...
```
Not very surprising, more obfuscated PowerShell, let's analyse it. `( $pSHome[21]+$PsHOme[30]+'x')` is just and obfuscated `IEX`, we can execute the third part of the payload directly to deobfuscate it. We obtain the following result:
```powershell
...
```
Finaly ! The payload execute two remote scripts. The first one is the **Invoke-AmsiBypass** function from [Nishang](https://github.com/samratashok/nishang) and the second one is used to reflectively inject a PE in memory.
